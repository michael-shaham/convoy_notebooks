...
class QuadDMPC(ControllerBase):
    """
    Platoon MPC controller with multi-objective tradeoff between squares of 
    norms of errors.
    ...
    def mpc_problem(self, x_0, y_a, info_set, y_i_a, d_i):
        ...
        for k in range(self.H):
            ...
            cost += cp.quad_form(y[:, k] - y_a[:, k], self.F)
            for j, i in enumerate(info_set):
                if i == 0:
                    cost += cp.quad_form(
                        y[:, k] - y_i_a[j][:, k] + d_i_tilde[j], self.Q
                    )
                else:
                    cost += cp.quad_form(
                        y[:, k] - y_i_a[j][:, k] + d_i_tilde[j], self.G
                    )
